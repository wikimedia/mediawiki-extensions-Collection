	...
	public function execute() {
		...
		<div class="collection-create-chapter-links">
			...
			<?php if ( count( $this->data['collection']['items'] ) > 0 ) { ?>
				<a class="collection-sort" href="<?php echo htmlspecialchars( SkinComponentUtils::makeSpecialUrl( 'Book', [ 'bookcmd' => 'sort_items' ] ) ) ?>"><?php $this->msg( 'coll-sort_alphabetically' ) ?></a>
				<a class="collection-clear" href="<?php echo htmlspecialchars( SkinComponentUtils::makeSpecialUrl( 'Book', [ 'bookcmd' => 'clear_collection' ] ) ) ?>"><?php $this->msg( 'coll-clear_collection' ) ?></a>
			<?php } ?>
		</div>
				...
				foreach ( $this->data['collection']['items'] as $index => $item ) {
					if ( $item['type'] == 'article' ) { ?>
						<li id="item-<?php echo intval( $index ) ?>" class="article">
							<a onclick="return coll_remove_item(<?php echo intval( $index ) ?>)" href="<?php echo htmlspecialchars( SkinComponentUtils::makeSpecialUrl( 'Book', [ 'bookcmd' => 'remove_item', 'index' => $index ] ) ) ?>" title="<?php $this->msg( 'coll-remove' ) ?>"><img src="<?php echo htmlspecialchars( $mediapath . "remove.png" ) ?>" width="10" height="10" alt="<?php $this->msg( 'coll-remove' ) ?>" /></a><a>
								<noscript>
									<?php if ( $index == 0 ) { ?>
										<img src="<?php echo htmlspecialchars( $mediapath . "trans.png" ) ?>" width="10" height="10" alt="" />
									<?php } else { ?>
										<a onclick="return coll_move_item(<?php echo intval( $index ) . ', -1' ?>)" href="<?php echo htmlspecialchars( SkinComponentUtils::makeSpecialUrl( 'Book', [ 'bookcmd' => 'move_item', 'delta' => '-1', 'index' => $index ] ) ) ?>" title="<?php $this->msg( 'coll-move_up' ) ?>"><img src="<?php echo htmlspecialchars( $mediapath . "up.png" ) ?>" width="10" height="10" alt="<?php $this->msg( 'coll-move_up' ) ?>" /></a>
									<?php }
			...
			echo htmlspecialchars( $item['title'] );
					...
					<?php } elseif ( $item['type'] == 'chapter' ) { ?>
						<li id="item-<?php echo intval( $index ) ?>" class="chapter">
							<a onclick="return coll_remove_item(<?php echo intval( $index ) ?>)" href="<?php echo htmlspecialchars( SkinComponentUtils::makeSpecialUrl( 'Book', [ 'bookcmd' => 'remove_item', 'index=' => $index ] ) ) ?>" title="<?php $this->msg( 'coll-remove' ) ?>"><img src="<?php echo htmlspecialchars( $mediapath . "remove.png" ) ?>" width="10" height="10" alt="<?php $this->msg( 'coll-remove' ) ?>" /></a>
							<noscript>
								<?php if ( $index == 0 ) { ?>
									<img src="<?php echo htmlspecialchars( $mediapath . "trans.png" ) ?>" width="10" height="10" alt="" />
								<?php } else { ?>
